...
object Case01 extends SparkHandler with ReflectHandler {
  ...
  def main(args: Array[String]): Unit = {
    ...
    invokeSessionHandler { () =>
      ...
      val rddPackage1000 = spark.sparkContext.parallelize(rddInstallCount___package_name.map(_._2).take(1000))
      ...
      rddCombinePackage1000 take(10) foreach println
      val t = rddCombinePackage1000.map { case(o, o_) =>
        val user = rddUser___installPackages.filter { case (u, ps) =>
          ps.exists(p => p == o && p == o_)
        }
        .map(_._1)
        .first
        (s"$o_$o_" -> user)
